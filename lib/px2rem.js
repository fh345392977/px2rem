...
var defaultConfig = {
  ...
  minSize: 4,        // rem value precision (default: 6)
      ...
      for (var j = 0; j < declarations.length; j++) {
        ...
        if (declaration.type === 'declaration' && pxRegExp.test(declaration.value)) {
          ...
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
            if (nextDeclaration.comment.trim() === config.keepComment || (parseFloat(declaration.value.substr(0, declaration.value.length - 2)) <= config.minSize)) { // no transform
